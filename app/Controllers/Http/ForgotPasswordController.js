...
class ForgotPasswordController {
  async store ({ request, response }) {
    try {
      ...
      await Mail.send(
        ...
        message => {
          message.to(user.email).from('lucasguidotti18@gmail.com', 'Lucas Guidotti').subject('Recuperação de senha')
        }
      )
    } catch (error) {
      return response
  ...
  async update ({ request, response }) {
    try {
      ...
      const tokenExired = moment().subtract('2', 'days').isAfter(user.token_created_at)
